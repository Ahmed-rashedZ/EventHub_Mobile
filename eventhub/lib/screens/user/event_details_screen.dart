...
class _EventDetailsScreenState extends State<EventDetailsScreen> {
  ...
  void initState() {
    super.initState();
    _avgRating =
        double.tryParse(widget.event['average_rating']?.toString() ?? '0') ?? 0;
    _loadReviews();
    _checkTicket();
  }
  ...
  void _checkTicket() {
    final tickets = Provider.of<TicketProvider>(
      context,
      listen: false,
    ).myTickets;
    ...
    if (data != null && mounted) {
      setState(() {
        ...
        if (data['average_rating'] != null) {
          _avgRating =
              double.tryParse(data['average_rating'].toString()) ?? _avgRating;
        }
  ...
  Widget build(BuildContext context) {
    ...
    final DateTime dt = dateStr != null
    ...
    final endSte = e['end_time'];
    DateTime? endDt;
    try {
      if (endSte != null) endDt = DateTime.parse(endSte.toString());
    } catch (_) {}
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildInfoCard(
                    ...
                    'Venue',
                    '$venueName${venueLocation.isNotEmpty ? '\n$venueLocation' : ''}',
                  ),
                  const SizedBox(height: 12),
                  _buildInfoCard(
                    Icons.people_outline,
                    'Capacity',
                    '${e['capacity']} attendees',
                  ),
                  if (e['creator'] != null) ...[
                    const SizedBox(height: 12),
                    _buildInfoCard(
  ...
  Widget _buildSponsorCard(Map<String, dynamic> sponsor) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [tColor.withValues(alpha: 0.08), AppColors.bgCard],
        ),
        borderRadius: BorderRadius.circular(14),
